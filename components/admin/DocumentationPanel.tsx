...
const AccordionItem: React.FC<AccordionItemProps> = ({ title, children, isOpen, onToggle }) => (
  <div className="border border-slate-200 dark:border-slate-700/80 rounded-lg">
    <button
      onClick={onToggle}
      className="w-full flex items-center justify-between p-4 text-left font-semibold text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg transition-colors"
      aria-expanded={isOpen}
    >
      <span>{title}</span>
      <ChevronRightIcon className={`h-5 w-5 text-slate-500 dark:text-slate-400 transition-transform ${isOpen ? 'rotate-90' : ''}`} />
    </button>
    {isOpen && (
      <div className="p-6 border-t border-slate-200 dark:border-slate-700/80 prose prose-slate dark:prose-invert max-w-none">
        {children}
      </div>
    )}
  </div>
                        ...
                        {
                            {
                                ...
                                socialmedia: <>
                                    ...
                                    <p><strong>API Access:</strong> Uses public RSS feeds generated by services like Nitter (for X) and YouTube's native RSS functionality.</p>
                                </>
                            }[service.id]
                        }
                    </div>
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Application architecture and integrated services overview.</p>
            </div>
