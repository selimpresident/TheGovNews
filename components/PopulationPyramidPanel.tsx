    ...
    if (!hasData) {
        return (
            <div className="text-center p-16">
                ...
                <h3 className="mt-4 text-xl font-semibold text-slate-800 dark:text-slate-100">No Population Data</h3>
                <p className="mt-2 text-slate-500 dark:text-slate-400 max-w-md mx-auto">{`Could not retrieve population pyramid data for ${countryName}.`}</p>
    ...
    return (
        <div>
            <div className="p-6 md:p-8 border-b border-slate-200 dark:border-slate-700">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Population Pyramid</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-1">{`Demographic breakdown for ${data.country} in ${data.year}.`}</p>
            </div>
            
            <div className="p-4 sm:p-6 md:p-8">
                <div className="mb-6 flex justify-between items-center bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Total Population</h3>
                    <div className="flex items-center gap-2">
                        <UsersIcon className="h-6 w-6 text-blue-600 dark:text-blue-500"/>
                        <p className="text-2xl font-bold text-slate-900 dark:text-slate-50">{formatNumber(data.totalPopulation)}</p>
                    </div>
                </div>

                <div className="w-full">
                    <div className="flex justify-between items-center mb-2 text-sm font-semibold text-slate-700 dark:text-slate-200">
                    ...
                    <div className="space-y-1">
                        {data.pyramid.map((group, index) => (
                           <div key={group.age} className="flex items-center justify-between group">
                               ...
                               <div className="w-[45%] flex justify-end items-center">
                                    <div 
                                        className="h-6 bg-blue-500/80 dark:bg-blue-500/60 rounded-l-md transition-all duration-300 group-hover:bg-blue-600 dark:group-hover:bg-blue-500"
                               ...
                               {/* Age Group Label */}
                               <div className="w-[10%] text-center text-xs font-mono text-slate-500 dark:text-slate-400 px-1">{group.age}</div>

                               {/* Female Bar */}
                               <div className="w-[45%] flex justify-start items-center">
                                    <div 
                                        className="h-6 bg-pink-500/80 dark:bg-pink-500/60 rounded-r-md transition-all duration-300 group-hover:bg-pink-600 dark:group-hover:bg-pink-500"
                     ...
                     <div className="flex justify-between items-center mt-2 text-xs font-mono text-slate-500 dark:text-slate-400">
             ...
             <Tooltip id="pyramid-tooltip" className="!bg-slate-900 !text-slate-200 rounded-md shadow-lg !border !border-slate-700 !text-sm" />
        </div>
    );
};
