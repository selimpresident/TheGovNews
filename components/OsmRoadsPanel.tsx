    ...
    if (isLoading) {
        return (
            <div className="flex flex-col justify-center items-center h-full min-h-[60vh]">
                <Spinner />
                <p className="mt-4 text-slate-500 dark:text-slate-400">Loading road network data...</p>
            </div>
        );
    }
    ...
    if (!hasData) {
        return (
            <div className="text-center p-16">
                <RoadIcon className="mx-auto h-12 w-12 text-slate-400 dark:text-slate-500"/>
                <h3 className="mt-4 text-xl font-semibold text-slate-800 dark:text-slate-100">No Road Data Available</h3>
                <p className="mt-2 text-slate-500 dark:text-slate-400 max-w-md mx-auto">{`Could not retrieve road network data for ${countryName}. The query might have been too complex or the region may have limited data.`}</p>
    ...
    return (
        <div>
            <div className="p-6 md:p-8 border-b border-slate-200 dark:border-slate-700">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Road Network</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-1">{`Major road networks in ${countryName} from OpenStreetMap.`}</p>
            </div>
            
            <div className="relative">
                <div className="absolute top-4 left-4 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-lg">
                    <h4 className="font-semibold text-sm mb-2 text-slate-800 dark:text-slate-200">Highway Types</h4>
                ...
                <div className="w-full h-[65vh] bg-slate-100 dark:bg-slate-900">
