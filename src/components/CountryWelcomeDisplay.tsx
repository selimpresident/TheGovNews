...
const CountryWelcomeDisplay: React.FC<CountryWelcomeDisplayProps> = ({ countryName, geography, worldAtlas, coordinates, countryMappings }) => {
    ...
    if (!geography || !worldAtlas || !coordinates) {
        return <div className="h-full flex items-center justify-center"><p className="text-analyst-text-secondary">Loading map...</p></div>;
    }
    ...
    const layerConfig: Record<Layer, { name: string; icon: React.ReactNode; color: string; data: any[] }> = {
        ...
        economy: { name: 'Economic Hotspots', icon: <ChartBarIcon className="h-5 w-5"/>, color: 'text-analyst-accent', data: mockEconomicHotspots },
    ...
    return (
        <div className="h-full flex flex-col items-center justify-center p-4 relative">
             <div className="absolute top-4 left-4 z-10 text-center">
                 <h1 className="text-4xl font-bold text-slate-800 dark:text-analyst-text-primary drop-shadow-lg">{greeting}!</h1>
             </div>
            <div className="absolute top-4 right-4 z-10 bg-white/80 dark:bg-analyst-sidebar/80 backdrop-blur-md p-3 rounded-lg border border-slate-200 dark:border-analyst-border shadow-lg">
                <h4 className="font-semibold text-sm mb-2 text-slate-800 dark:text-analyst-text-primary">Toggle Layers</h4>
                 <div className="space-y-2">
                    {Object.entries(layerConfig).map(([key, config]) => (
                        <label key={key} className={`flex items-center gap-3 cursor-pointer p-2 rounded-md transition-colors ${activeLayers.has(key as Layer) ? 'bg-slate-200/50 dark:bg-analyst-item-active' : ''}`}>
                            <input
                                type="checkbox"
                            ...
                            <div className={`flex items-center gap-2 ${config.color}`}>
                                {config.icon}
                                <span className="text-sm font-medium text-slate-700 dark:text-analyst-text-primary">{config.name}</span>
                            </div>
                        </label>
                    ))}
                 </div>
                 {isLoading && <div className="absolute inset-0 bg-white/50 dark:bg-analyst-sidebar/50 flex items-center justify-center"><Spinner /></div>}
            </div>
            ...
            >
                    ...
                    {({ geographies }) =>
                      geographies.map(geo => (
                        <Geography
                            ...
                            stroke="#f8fafc"
                            className={`dark:stroke-analyst-dark-bg ${geo.properties.name === geography.properties.name ? 'dark:fill-analyst-item-active' : 'dark:fill-analyst-sidebar/80'}`}
                        />
                      ))
                    }
                </Geographies>
                ...
                {activeLayers.has('conflicts') && conflictData.map(point => (
                 ...
                 {activeLayers.has('economy') && mockEconomicHotspots.map(point => (
                     <Marker key={point.id} coordinates={point.coordinates}>
                        <circle r={6} className="fill-analyst-accent/50 stroke-analyst-accent/80" data-tooltip-id="map-tooltip" data-tooltip-content={point.name} />
                    </Marker>
                ))}
            ...
            <Tooltip id="map-tooltip" className="!bg-analyst-sidebar !text-analyst-text-primary rounded-md shadow-lg !border !border-analyst-border !text-sm z-20" />
        </div>
    );
};
