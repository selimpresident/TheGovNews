    ...
    return (
        <div className="h-full flex flex-col">
            <div className="p-5 border-b border-slate-200 dark:border-analyst-border flex-shrink-0">
                <h2 className="text-lg font-bold text-slate-900 dark:text-analyst-text-primary">Service Monitoring</h2>
                <p className="text-sm text-slate-500 dark:text-analyst-text-secondary">Real-time status of all integrated third-party services.</p>
            </div>
            <div className="p-6 overflow-y-auto flex-grow">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {services.map(service => {
                        ...
                        return (
                            <div key={service.id} className={`p-5 rounded-lg border border-slate-200 dark:border-analyst-border ${style.bg}`}>
                                <div className="flex justify-between items-center mb-3">
                                    <h3 className="font-bold text-lg text-slate-900 dark:text-analyst-text-primary">{service.name}</h3>
                                ...
                                <div className="space-y-2 text-sm text-slate-600 dark:text-analyst-text-secondary">
                                    <div className="flex justify-between">
                                        <span>Avg. Response:</span>
                                        <span className="font-medium text-slate-800 dark:text-analyst-text-primary">{service.avgResponseTime}ms</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span>Uptime (30d):</span>
                                        <span className="font-medium text-slate-800 dark:text-analyst-text-primary">{service.uptime.toFixed(2)}%</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span>Last Success:</span>
                                        <span className="font-medium text-slate-800 dark:text-analyst-text-primary">{new Date(service.lastSuccess).toLocaleTimeString()}</span>
                                    </div>
                                </div>
                            </div>
                        );
