        ...
        <a href={article.url} target="_blank" rel="noopener noreferrer" className="block p-5 transition-colors hover:bg-slate-50/50 dark:hover:bg-analyst-item-hover">
            <div className="flex justify-between items-start gap-4">
                <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 dark:text-analyst-text-secondary">{article.domain}</p>
                </div>
                <p className="text-sm text-slate-500 dark:text-analyst-text-secondary whitespace-nowrap pl-4">{formatDateString(article.seendate)}</p>
            </div>
        </a>
    );
};
...
const GdeltInfoPanel: React.FC<{ articles: GdeltArticle[], countryName: string, error?: string | null }> = ({ articles, countryName, error }) => {
        ...
        return (
            <div className="flex-grow flex items-center justify-center p-16 text-center">
                <div>
                    ...
                    <h3 className="mt-4 text-xl font-semibold text-slate-800 dark:text-analyst-text-primary">Error Loading Media Data</h3>
                    <p className="mt-2 text-slate-500 dark:text-analyst-text-secondary max-w-md mx-auto">{`Could not retrieve global media data for ${countryName}.`}</p>
        ...
        return (
            <div className="flex-grow flex items-center justify-center p-16 text-center">
                <div>
                    ...
                    <h3 className="mt-4 text-xl font-semibold text-slate-800 dark:text-analyst-text-primary">No Global Media Data</h3>
                    <p className="mt-2 text-slate-500 dark:text-analyst-text-secondary">{`No recent articles mentioning ${countryName} were found in the GDELT database.`}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="divide-y divide-slate-200/70 dark:divide-analyst-border/50 overflow-y-auto">
